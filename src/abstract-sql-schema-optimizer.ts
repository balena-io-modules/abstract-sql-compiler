...
const removeExistChecksForRequiredReferencedFields = (
	...
	for (const n of nodes) {
		...
		if (n[0] === 'Exists' && Array.isArray(n[1])) {
			...
			if (typeof fieldName === 'string') {
				if (fieldsByFieldName[fieldName]?.required !== true) {
					result.push(n);
				}
				continue;
			}
		...
		let newNode = [
			n[0],
			...removeExistChecksForRequiredReferencedFields(
				resourceAlias,
				fieldsByFieldName,
				n.slice(1) as BooleanTypeNodes[],
			),
		] as BooleanTypeNodes;
		// Remove unnecessary AND nodes
		if (newNode[0] === 'And') {
			if (newNode.length === 1) {
				// We removed all its child nodes, so skip adding an empty AND in the result.
				continue;
			}
			if (newNode.length === 2) {
				// Only a single child is left, so we unwrap its content.
				newNode = newNode[1];
			}
		}
		result.push(newNode);
...
function convertRuleToPartialUniqueIndex(
	...
	// are matched with the respective table of the outer query.
	...
	// Other than the nodes like `"resource.inner"."fieldX" = "resource.outer"."fieldX"`,
	// there should only be`a single `"resource.inner"."fieldX" = "parent.0"."<idField>"` node
	// only when the is a parent FROM in the query.
