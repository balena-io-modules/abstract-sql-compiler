...
// No dedicated range-type nodes yet; placeholder for future range literals /
// range-producing nodes that would be valid inputs to RangeLower / RangeUpper.
export type StrictRangeTypeNodes = never;
// export type RangeTypeNodes = StrictRangeTypeNodes | UnknownTypeNodes;
// using above union while StrictRangeTypeNodes = never triggers redundant-type errors)
...
export const isWhereNode = (n: AbstractSqlType): n is WhereNode =>
	n[0] === 'Where';
export const isReferencedFieldNode = (
	n: AbstractSqlType,
): n is ReferencedFieldNode => n[0] === 'ReferencedField';
export const isFieldNode = (n: AbstractSqlType): n is FieldNode =>
	n[0] === 'Field';
export const isFieldTypeNode = (
	n: AbstractSqlType,
): n is FieldNode | ReferencedFieldNode =>
	isReferencedFieldNode(n) || isFieldNode(n);
