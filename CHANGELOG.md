...
* Support the `Or` -> `In` optimisation for cases where you have mixed comparisons in an or block, allowing it to be used in a lot more cases. #20 [Pagan Gazzard]
* Make sure the nested or optimisation hits the validator method directly, avoiding other optimisations breaking it. #20 [Pagan Gazzard]
